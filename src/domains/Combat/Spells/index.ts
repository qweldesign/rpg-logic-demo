...
export type SpellEffect =
  ...
  | { kind: 'trip' }
  | { kind: 'dmg', dice: number, dmgType: number, metalPenalty?: boolean, randomTarget?: boolean }
  | { kind: 'dmgAll', dice: number, dmgType: number }
  | { kind: 'flash' }
...
const BLUE_SPELL: Spell[] = [
  { id: 0, label: '生命の雫', cast: 1, effects: [{ kind: 'heal', fraction: 1 / 3 }], targetScope: 'ally' },
