      ...
      <div className="actions confirm" data-disable={actionPalette !== 'confirmAttack'}>
        {actionTarget && (
          <div className="confirm__grid">
            ...
            <div className={actionTarget === store.actor.attack.feint?.target ? 'is-targeted' : ''}>防御目標値: {actionTarget.defense.canDefend ? `${actionTarget.defense.getTarget(store.actor).target} (${defenseType[actionTarget.defense.getTarget(store.actor).type]})` : '防御不能'}</div>
            <div>効果: </div>
